...
public class SortUilts {
	...
	public static void merge(VCFSortOptions options, Configuration conf) {
		try {
			...
			for(String result : options.getMultiOutputPath().values()){
            ...
            for( long id : multiOutputs.keySet()){
            	VCFHeader newHeader = options.getHeaderID().get(id);
