...
public class MannWhitneyU {
    ...
    public Pair<Long,Long> getR1R2() {
        long u1 = calculateOneSidedU(observations, USet.SET1);
    ...
    public static Pair<Double,Double> calculatePRecursively(int n, int m, long u, boolean twoSided, ExactMode mode) {
        if ( m > 8 && n > 5 ) { throw new UserException(String.format("Please use the appropriate (normal or sum of uniform) approximation. Values n: %d, m: %d",n,m)); }
        ...
        try {
            ...
            throw new UserException("A math exception occurred in inverting the probability", me);
        }
    ...
    private static class DitheringComparator implements Comparator<Pair<Number,USet>>, Serializable {
		 ...
		 * 
		 */
		private static final long serialVersionUID = 6082338129837694507L;
		private  Random random = RandomUtils.getNewRandomGenerator();
    ...
    public static void main(String[] args)
    {
    	...
    	//           32.0 ,30.0 ,29.0 ,29.0 ,15.0 ,31.0 ,29.0 ,31.0 ,30.0 ,30.0 ,30.0 ,32.0 ,32.0 ,32.0 ,31.0 ,29.0
    	double rq[]={32.0 ,30.0 ,29.0 ,29.0 ,15.0 ,31.0 ,29.0 ,31.0 ,30.0 ,30.0, 30.0 ,32.0 ,32.0 ,32.0 ,31.0 ,29.0};
    	for(double r:rq)
    		refQuals.add(r);
    	//           31.0 ,32.0 ,31.0 ,31.0 ,32.0 ,32.0 ,32.0 ,31.0 ,32.0 ,32.0 ,32.0 ,30.0 ,32.0 ,32.0
    	double[] aq={31.0 ,32.0 ,31.0 ,31.0 ,32.0 ,32.0 ,32.0 ,31.0 ,32.0 ,32.0 ,32.0 ,30.0 ,32.0 ,32.0};
    	
    	for(double a :aq)
    	altQuals.add(a);
    	 final MannWhitneyU mannWhitneyU = new MannWhitneyU(true);
         for (final Double qual : altQuals) {
             mannWhitneyU.add(qual, USet.SET1);
         }
         for (final Double qual : refQuals) {
             mannWhitneyU.add(qual, USet.SET2);
         }

         final Pair<Double, Double> testResults = mannWhitneyU.runOneSidedTest(USet.SET1);
