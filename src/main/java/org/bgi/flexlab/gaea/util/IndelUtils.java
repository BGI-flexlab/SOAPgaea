...
public class IndelUtils {
    ...
    public static ArrayList<Integer> findEventClassificationIndex(VariantContext vc, ChromosomeInformationShare ref, int position) {
        int eventLength;
        ...
        //FIXME:: wrong when at edges of chromosome
    ...
    public static String getIndelClassificationName(int k) {
        if (k >=0 && k < COLUMN_KEYS.length)
            ...
            throw new UserException("Invalid index when trying to get indel classification name");
    }

    public static boolean isInsideExtendedIndel(VariantContext vc, int position) {
        return (vc.getStart() != position);
    }
}
