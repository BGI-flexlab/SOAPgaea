...
public final class SAMUtils
{
    ...
    public static void makeReadUnmapped() {}
     ...
     */
    public static boolean cigarMapsNoBasesToRef() {
		return false;
		}

	public static  String reverseQ(String quality)
	{
		byte[] QUAL=quality.getBytes();
		byte tem;	
		for (int i = 0,len=QUAL.length-1; i < len; i++,len--) {
			tem=QUAL[i];
			QUAL[i]=QUAL[len];
			QUAL[len]=tem;
		}
	...
	public static String reverseB(String base)
	{
		HashMap<Byte, Byte> R=new HashMap<Byte, Byte>();
		byte A='A';
		byte T='T';
		byte C='C';
		byte G='G';
		...
		byte[] BASE=base.getBytes();
		byte tem;
		for (int i = 0,len=BASE.length-1; i <= len; i++,len--) {
			tem=BASE[i];
			BASE[i]=R.get(BASE[len]);
			BASE[len]=R.get(tem);
		}
		return new String(BASE);
	}
    
	public static boolean isUnmappedRead(SAMRecord sam)
	{
		if ((sam.getFlags()&0x04)>0) {
			return true;
		}else {
			return false;
		}
