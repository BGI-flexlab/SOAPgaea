...
public abstract class VCFCompoundHeaderLine extends VCFHeaderLine implements VCFIDHeaderLine {
    ...
    public int getCount() {
        if (!isFixedCount()) {
            throw new UserException("Asking for header line count when type is not an integer");
        }
        return count;
    }
    ...
    public int getCount(final VariantContext vc) {
        switch ( countType ) {
            ...
            case G:
                final int ploidy = vc.getMaxPloidy(0);
                return GenotypeLikelihoods.numLikelihoods(vc.getNAlleles(), ploidy == 0 ? 2 : ploidy);
            default:
                throw new UserException("Unknown count type: " + countType);
        }
    }
