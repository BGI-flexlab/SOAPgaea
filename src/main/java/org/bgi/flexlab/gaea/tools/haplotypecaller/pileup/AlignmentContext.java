...
public final class AlignmentContext implements Locatable {
	// Definitions:
	...
	/**
	 * Returns a potentially derived subcontext containing only forward,
	 * reverse, or in fact all reads in alignment context context.
	 */
	public AlignmentContext stratify(final ReadOrientation type) {
		...
		case COMPLETE:
			return this;
		case FORWARD:
			return new AlignmentContext(loc,
					basePileup.makeFilteredPileup(pe -> !pe.getRead().getReadNegativeStrandFlag()));
		case REVERSE:
			return new AlignmentContext(loc,
					basePileup.makeFilteredPileup(pe -> pe.getRead().getReadNegativeStrandFlag()));
		default:
	...
	/**
	 * Splits the given AlignmentContext into a StratifiedAlignmentContext per
	 * sample, but referenced by sample name instead of sample object.
	 *
	 * @param assumedSingleSample
	 *            If non-null, assume this is the only sample in our pileup and
	 *            return immediately. If null, get the list of samples from the
	 *            provided header and do the work of splitting by sample.
	 * @return a Map of sample name to StratifiedAlignmentContext; samples
	 *         without coverage are not included
	 **/
	public Map<String, AlignmentContext> splitContextBySampleName(final String assumedSingleSample,
		...
		// this will throw an user error if there are samples without
		// RG/sampleName
	...
	public static Map<String, AlignmentContext> splitContextBySampleName(final ReadPileup pileup,
			final SAMFileHeader header) {
