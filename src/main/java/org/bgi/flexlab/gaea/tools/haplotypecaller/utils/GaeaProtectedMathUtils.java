...
public class GaeaProtectedMathUtils implements Serializable {

    private static final long serialVersionUID = -7587940242569731513L;

    private GaeaProtectedMathUtils() {
    }
    ...
    public static double logSumExp(final double ... values) {
        double max = GvcfMathUtils.arrayMax(Utils.nonNull(values));
        double sum = 0.0;
    ...
    public static double mean(final double ... values) {
        ...
        return GvcfMathUtils.mean(values, 0, values.length);
    }
    ...
    public static double[] columnSums(final RealMatrix matrix) {
        ...
        return IntStream.range(0, matrix.getColumnDimension())
                .mapToDouble(c -> GvcfMathUtils.sum(matrix.getColumn(c))).toArray();
    }
    ...
    public static double[] rowSums(final RealMatrix matrix) {
        ...
        return IntStream.range(0, matrix.getRowDimension())
                .mapToDouble(r -> GvcfMathUtils.sum(matrix.getRow(r))).toArray();
    }
    ...
    public static double[] posteriors(double[] log10Priors, double[] log10Likelihoods) {
        return GvcfMathUtils.normalizeFromLog10ToLinearSpace(MathArrays.ebeAdd(log10Priors, log10Likelihoods));
    }
    ...
    public static double dotProduct(double[] a, double[] b){
        return GvcfMathUtils.sum(MathArrays.ebeMultiply(a, b));
    }
    ...
    public static double logProbToPhredScore(final double rawLogProb, final boolean complement,
        ...
        if (rawLogProb > maxLogProb) {
            throw new UserException(String.format("possible numerical instability problem: the log-probability is too" +
