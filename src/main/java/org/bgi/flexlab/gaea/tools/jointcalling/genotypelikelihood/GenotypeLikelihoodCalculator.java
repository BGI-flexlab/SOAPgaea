...
public class GenotypeLikelihoodCalculator {
    ...
    private void manyComponentGenotypeLikelihoodByRead(final GenotypeAlleleCounts genotypeAlleleCounts,
        ...
        for (int r = 0, readDataOffset = 0; r < readCount; r++, readDataOffset += maximumDistinctAllelesInGenotype)
            likelihoodByRead[r] = approximateLog10SumLog10(readGenotypeLikelihoodComponents, readDataOffset, readDataOffset + componentCount);
    }
    ...
    public static double approximateLog10SumLog10(final double[] vals, final int fromIndex, final int toIndex) {
        ...
        final int maxElementIndex = maxElementIndex(vals,fromIndex,toIndex);
    ...
    private static class JacobianLogTable {
        ...
        public static double get(final double difference) {
            if (cache == null)
                initialize();
            final int index = MathUtils.fastRound(difference * INV_STEP);
            return cache[index];
        }
