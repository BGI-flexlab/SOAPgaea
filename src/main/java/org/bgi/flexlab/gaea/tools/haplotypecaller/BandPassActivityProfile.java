...
public class BandPassActivityProfile extends ActivityProfile {
     ...
     */
    public BandPassActivityProfile(final Set<GenomeLocation> restrictToIntervals,
     ...
     */
    public BandPassActivityProfile(final Set<GenomeLocation> restrictToIntervals, final int maxProbPropagationDistance,
        ...
        for ( final ActivityProfileState superState : super.processState(justAddedState) ) {
            if ( superState.isActiveProb() > 0.0 ) {
                for( int i = -filterSize; i <= filterSize; i++ ) {
                    final GenomeLocation loc = getLocForOffset(justAddedState.getLoc(), i);
                    if ( loc != null ) {
