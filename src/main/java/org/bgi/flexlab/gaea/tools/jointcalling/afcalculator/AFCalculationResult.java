...
public class AFCalculationResult {
	private final static int AF0 = 0; // index of the AC=0 entry in the log10
										// array (of priors, likelihoods, or
										// posteriors listed below)
	private final static int AF1p = 1; // index of the AC>0 entry in the log10
										// array
	...
	/**
	 * Create a results object capability of storing results for calls with up
	 * to maxAltAlleles
	 */
	public AFCalculationResult(final int[] alleleCountsOfMLE, final int nEvaluations,
			final List<Allele> allelesUsedInGenotyping, final double[] log10LikelihoodsOfAC,
			final double[] log10PriorsOfAC, final Map<Allele, Double> log10pRefByAllele) {
		if (allelesUsedInGenotyping == null || allelesUsedInGenotyping.size() < 1)
			throw new IllegalArgumentException(
					"allelesUsedInGenotyping must be non-null list of at least 1 value " + allelesUsedInGenotyping);
		...
		if (alleleCountsOfMLE.length != allelesUsedInGenotyping.size() - 1)
			throw new IllegalArgumentException("alleleCountsOfMLE.length " + alleleCountsOfMLE.length
					+ " != number of alternate alleles used in genotyping " + (allelesUsedInGenotyping.size() - 1));
		if (nEvaluations < 0)
		...
		if (log10pRefByAllele.size() != allelesUsedInGenotyping.size() - 1)
			throw new IllegalArgumentException("log10pRefByAllele has the wrong number of elements: log10pRefByAllele "
		...
		if (!allelesUsedInGenotyping.containsAll(log10pRefByAllele.keySet()))
			throw new IllegalArgumentException(
					"log10pRefByAllele doesn't contain all of the alleles used in genotyping: log10pRefByAllele "
							+ log10pRefByAllele + " but allelesUsedInGenotyping " + allelesUsedInGenotyping);
	...
	public AFCalculationResult withNewPriors(final double[] log10PriorsOfAC) {
		return new AFCalculationResult(alleleCountsOfMLE, nEvaluations, allelesUsedInGenotyping, log10LikelihoodsOfAC,
				log10PriorsOfAC, log10pRefByAllele);
	}

	/**
	 * Returns a vector with maxAltAlleles values containing AC values at the
	 * MLE
	 *
	 * The values of the ACs for this call are stored in the
	 * getAllelesUsedInGenotyping order, starting from index 0 (i.e., the first
	 * alt allele is at 0). The vector is always maxAltAlleles in length, and so
	 * only the first getAllelesUsedInGenotyping.size() - 1 values are
	 * meaningful.
	 *
	 ...
	 *
	 * @param allele
	 *            the allele whose AC we want to know. Error if its not in
	 *            allelesUsedInGenotyping
	 * @throws IllegalStateException
	 *             if allele isn't in allelesUsedInGenotyping
	...
	/**
	 * Returns the number of cycles used to evaluate the pNonRef for this AF
	 * calculation
	 *
	 * @return the number of evaluations required to produce the answer for this
	 *         AF calculation
	 */
	 ...
	 *
	 * Due to computational / implementation constraints this may be smaller
	 * than the actual list of alleles requested
	 *
	...
	/**
	 * Get the log10 normalized -- across all ACs -- posterior probability of AC
	 * == 0 for all alleles
	 *
	 * @return
	 */
	...
	/**
	 * Get the log10 unnormalized -- across all ACs -- likelihood of AC == 0 for
	 * all alleles
	 *
	 * @return
	 */
	...
	/**
	 * Get the log10 unnormalized -- across all ACs -- prior probability of AC
	 * == 0 for all alleles
	 *
	 * @return
	 */
	...
	public String toString() {
		...
		for (final Allele a : getAllelesUsedInGenotyping())
			if (a.isNonReference())
				byAllele.add(String.format("%s => MLE %d / posterior %.2f", a, getAlleleCountAtMLE(a),
						getLog10PosteriorOfAFEq0ForAllele(a)));
		return String.format("AFCalc%n\t\tlog10PosteriorOfAFGT0=%.2f%n\t\t%s", getLog10LikelihoodOfAFGT0(),
				Utils.join("\n\t\t", byAllele));
	}

	/**
	 * Are we sufficiently confidence in being non-ref that the site is
	 * considered polymorphic?
	 *
	 * We are non-ref if the probability of being non-ref > the emit confidence
	 * (often an argument). Suppose posterior AF > 0 is log10: -5 => 10^-5 And
	 * that log10minPNonRef is -3. We are considered polymorphic since 10^-5 <
	 * 10^-3 => -5 < -3
	 *
	 * Note that log10minPNonRef is really the minimum confidence, scaled as an
	 * error rate, so if you want to be 99% confidence, then log10PNonRef should
	 * be log10(0.01) = -2.
	 *
	 * @param log10minPNonRef
	 *            the log10 scaled min pr of being non-ref to be considered
	 *            polymorphic
	 *
	 ...
	 *
	 * @param log10minPNonRef
	 *            the confidence threshold, in log10 space
	 ...
	 * internally the log10 value of AF == 0, which grows very quickly negative
	 * and yet has sufficient resolution for high confidence tests. For example,
	 * if log10pRef == -100, not an unreasonably high number, if we tried to
	 * store log10pNonRef we'd be looking at 1 - 10^-100, which quickly
	 * underflows to 1. So the logic here is backward from what you really want
	 * (the p of segregating) but we do that for numerical reasons
	 *
	 * Unlike the sites-level annotation, this calculation is specific to
	 * allele, and can be used to separately determine how much evidence there
	 * is that allele is independently segregating as opposed to the site being
	 * polymorphic with any allele. In the bi-allelic case these are obviously
	 * the same but for multiple alt alleles there can be lots of evidence for
	 * one allele but not so much for any other allele
	 *
	 * @param allele
	 *            the allele we're interested in, must be in
	...
	/**
	 * Returns the log10 normalized posteriors given the log10 likelihoods and
	 * priors
	 *
	 ...
	 *
	 * Things like our MLE allele count vector are indexed by alt allele index,
	 * with the first alt allele being 0, the second 1, etc. This function
	 * computes the index associated with allele.
	 *
	 * @param allele
	 *            the allele whose alt index we'd like to know
