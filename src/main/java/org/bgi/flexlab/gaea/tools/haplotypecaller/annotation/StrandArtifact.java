...
import static org.bgi.flexlab.gaea.tools.haplotypecaller.annotation.StrandArtifact.StrandArtifactZ.*;
...
import org.bgi.flexlab.gaea.tools.haplotypecaller.utils.IntegrationUtils;
...
public class StrandArtifact extends GenotypeAnnotation implements StandardMutectAnnotation {
    ...
    public void annotate(final ChromosomeInformationShare ref,
        ...
        final double[] tumorLods = GaeaProtectedVariantContextUtils.getAttributeAsDoubleArray(vc, GaeaVCFConstants.TUMOR_LOD_KEY, () -> null, -1);
        final int indexOfMaxTumorLod = GvcfMathUtils.maxElementIndex(tumorLods);
        ...
        final int numFwdAltReads = (int) bestAlleles.stream().filter(ba -> !ba.read.getReadNegativeStrandFlag() && ba.isInformative() && ba.allele.equals(altAlelle)).count();
        final int numRevAltReads = (int) bestAlleles.stream().filter(ba -> ba.read.getReadNegativeStrandFlag() && ba.isInformative() && ba.allele.equals(altAlelle)).count();
        final int numFwdReads = (int) bestAlleles.stream().filter(ba -> !ba.read.getReadNegativeStrandFlag() && ba.isInformative()).count();
        final int numRevReads = (int) bestAlleles.stream().filter(ba -> ba.read.getReadNegativeStrandFlag() && ba.isInformative()).count();
        ...
        final double[] posterior_probabilities = GvcfMathUtils.normalizeFromRealSpace(
        ...
        final double[] samplePoints = GaeaProtectedMathUtils.createEvenlySpacedPoints(0.0, 1.0, numSamplePoints);
        ...
        final int maxAlleleFractionIndexFwd = GvcfMathUtils.maxElementIndex(likelihoodsGivenForwardArtifact);
        final int maxAlleleFractionIndexRev = GvcfMathUtils.maxElementIndex(likelihoodsGivenReverseArtifact);
    ...
    private double getIntegrandGivenNoArtifact(final double f, final int nPlus, final int nMinus, final int xPlus, final int xMinus){
        return GvcfMathUtils.binomialProbability(nPlus, xPlus, f) * GvcfMathUtils.binomialProbability(nMinus, xMinus, f);
    }
    ...
    private double getIntegrandGivenArtifact(final double f, final double epsilon, final int nWithArtifact, final int nNoArtifact, final int xWithArtifact, final int xNoArtifact){
        return betaPrior.density(epsilon) * GvcfMathUtils.binomialProbability(nWithArtifact, xWithArtifact, f + epsilon * (1-f)) *
                GvcfMathUtils.binomialProbability(nNoArtifact, xNoArtifact, f);
    }
