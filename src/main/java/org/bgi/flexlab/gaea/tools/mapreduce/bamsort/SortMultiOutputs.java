// From https://issues.apache.org/jira/secure/attachment/12418584/patch-370-5.txt
//
// With two changes to compile with Hadoop 0.20.2: replaced
// FileOutputFormat.PART with "part" and FileOutputFormat.setOutputName(x) with
// configuration.set("mapreduce.output.basename", x).
/**
 ...
 * group is the {@link MultipleOutputs} class name. The names of the counters
 * are the same as the output name. These count the number records written to
 * each output name.
 * </p>
 *
 ...
 * </pre>
 */
public class SortMultiOutputs<KEYOUT, VALUEOUT> {
	...
	private RecordWriter sortWriter = null;
	private String currentSample = null;
	private long[] times = new long[4];

	/**
	 * Counters group used by the counters of MultipleOutputs.
	 */
	private static final String COUNTERS_GROUP = SortMultiOutputs.class
			.getName();

	private TaskAttemptContext taskContext = null;
	private String namedOutputTmp = null;
	 ...
	 *
	 * The counters group is the {@link MultipleOutputs} class name. The names
	 * of the counters are the same as the named outputs. These counters count
	 * the number records written to each output name. By default these counters
	 * are disabled.
	 *
	 * @param job
	...
	/**
	 * Creates and initializes multiple outputs support, it should be
	 * instantiated in the Mapper/Reducer setup method.
	 *
	 * @param context
	 *            the TaskInputOutputContext object
	 */
	public SortMultiOutputs(
			TaskInputOutputContext<?, ?, KEYOUT, VALUEOUT> context) {
		this.context = context;
		namedOutputs = Collections.unmodifiableSet(new HashSet<String>(
				SortMultiOutputs.getNamedOutputsList(context)));
	...
	public <K, V> void write(String namedOutput, K key, V value,
		...
		if (namedOutputTmp == null || !namedOutputTmp.equals(namedOutput)) {
			...
			taskContext = getContext(namedOutput);
			namedOutputTmp = namedOutput;
		}
	...
	private synchronized RecordWriter newInstance(
		...
		if (countersEnabled) {
			String countName = "Reduce output records";
			/*sortWriter = new RecordWriterWithCounter(sortWriter, baseFileName,
					context);*/
			sortWriter = new RecordWriterWithCounter(sortWriter,countName,context);
		}
		return sortWriter;
	}
	...
	private synchronized RecordWriter getRecordWriter(
		...
		if (currentSample == null) {
			currentSample = baseFileName;
			// look for record-writer in the cache
			return newInstance(taskContext, baseFileName);
		} else {
			if (!baseFileName.equals(currentSample)) {
				currentSample = baseFileName;
				if (sortWriter != null)
					sortWriter.close(taskContext);
				return newInstance(taskContext, baseFileName);
			}
		}
	...
	private TaskAttemptContext getContext(String nameOutput) throws IOException {
		// The following trick leverages the instantiation of a record writer
		// via
	 ...
	 *
	 * This should be called from cleanup method of map/reduce task. If
	 * overridden subclasses must invoke <code>super.close()</code> at the end
	...
	public void close() throws IOException, InterruptedException {
		 ...
		 * writer.close(context); }
		 */
		System.err.println(times[0] + "\t" + times[1] + "\t" + times[2] + "\t"
				+ times[3]);
		if (sortWriter != null)
			sortWriter.close(context);
	}
}
