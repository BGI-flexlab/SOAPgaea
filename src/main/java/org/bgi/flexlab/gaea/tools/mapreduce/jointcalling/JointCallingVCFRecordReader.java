...
public class JointCallingVCFRecordReader extends RecordReader<LongWritable, VariantContextWritable> {
	...
	public void initialize(InputSplit spl, TaskAttemptContext ctx) throws IOException {
		...
		//System.err.println("split path:"+file.getName());
		...
		if (GZipType.isGzip(file)) {
			isGzip = true;
			reader = new AsciiLineReader(new GZIPInputStream(ins));
		} else
		...
		MultipleVCFHeaderForJointCalling multiHeaders = new MultipleVCFHeaderForJointCalling();
		fileIndex = multiHeaders.getIndex(ctx.getConfiguration(),file.getName());
		if(fileIndex == -1) {
			throw new UserException(file.getName()+" is not indexed.");
		}
	...
	public boolean nextKeyValue() throws IOException {
		if ( !isGzip && (!it.hasNext() || it.getPosition() >= length))
			return false;
		
		if ( isGzip && !it.hasNext())
			return false;
		...
		if (!info.hasAttribute("SM"))
			info.putAttribute("SM", fileIndex);
